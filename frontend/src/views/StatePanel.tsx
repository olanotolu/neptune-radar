...
interface StatePanelProps {
  ...
  stateName: string;
  onClose: () => void;
  // Layer + pin toggle are lifted to MapView so the map can render the
  // active layer's pins without duplicating the selection state.
  layer: LayerId;
...
export function StatePanel({ stateAbbrev, stateName, onClose, layer, setLayer, showPins, setShowPins }: StatePanelProps) {
  ...
  const overview = useStateOverview(stateAbbrev, true);
  const government = useStateGovernment(stateAbbrev, true);
  const churches = useStateChurches(stateAbbrev, true);
  const social = useStateSocial(stateAbbrev, true);
  ...
  return (
    <aside className="state-panel ohio-panel">
      <header className="state-panel__header">
        ...
        <label className="state-panel__pins-toggle">
          <input
            type="checkbox"
            checked={showPins}
      ...
      <div className="state-panel__body">
        ...
        {layer === "government" && (
          <GovernmentBody government={government} selectedCountyFips={selectedCountyFips} onSelectCounty={setSelectedCountyFips} />
        )}
        {layer === "churches" && (
          <ChurchesBody churches={churches} selectedDioceseId={selectedDioceseId} onSelectDiocese={setSelectedDioceseId} />
        )}
        {layer === "social" && <SocialBody social={social} />}
      </div>
...
function SocialBody({ social }: { social: { data?: SocialMarketView[]; isLoading: boolean } }) {
  if (social.isLoading) return <div className="ohio-empty">Loading social accounts…</div>;
  const markets = social.data ?? [];
